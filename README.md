...
## Trajectory Generation (Training Data)

> **Proven production workflow: 21 trajectories generated, 100% success rate, 0 failures.**

GLM-5.2 can generate high-quality multi-turn AI agent trajectories (ChatML format)
suitable for fine-tuning smaller models (Qwen3-4B, Llama-3.2-3B). The generated
trajectories include real code blocks, `<think>` reasoning, tool calls, and
environment responses.

### Batch Results (June 2026)

| Metric | Value |
|--------|-------|
| **Total generated** | 21 trajectories |
| **Success rate** | 100% (0 stuck, 0 timeout) |
| **Batch size** | 10 per session (2 sessions) |
| **Time per item** | 2–4 min (median ~3 min) |
| **Output size** | 5,206–25,005 chars (avg 13.8K) |
| **Total output** | ~317 KB across 21 items |

### Quick Run

```bash
# 1. Prepare contracts file (remaining_contracts.json)
# 2. Find active panel ID:
curl -s http://localhost:9222/json | python3 -c "
import sys,json
panels=[t for t in json.load(sys.stdin) if 'panel/' in t.get('url','')]
print(panels[0]['webSocketDebuggerUrl']) if panels else print('NO PANEL')
"

# 3. Edit CDP_WS in examples/trajectory_batch.py with the URL
# 4. Run:
python3 examples/trajectory_batch.py
```

### Key Engineering Insights

**Stability check:** 4 consecutive same-length polls (not 3). Deep Think output
appears first, then the actual response — the extra poll accounts for the gap.

**Baseline management:** Re-read `document.body.innerText.length` BEFORE each
send, because the body accumulates across items in the same chat session.

**UI chrome markers** to strip: `"New Tab"`, `"GLM-5.2"`, `"Expand"`,
`"Thinking Process"`, `"Deep Think"`, `"Web Search"`, `"New Chat"`, and
Tabbit URLs (`https://web.tabbit.ai/newtab`).

**Panel preference:** Always use `panel/<uuid>` (active chat), never
`panel?mode=mi` (dead launch screen).

**Format from GLM-5.2:** Uses `<|im_start|>tool \{json\}</|im_end|>` instead
of `<tool_call>\{json\}</tool_call>`. Post-processing may be needed to
...
| Failure | Symptom | Fix |
|---------|---------|-----|
| "Generating, wait" stuck | Prompt echoes, no response, body stable | `pkill -9 -f Tabbit` → restart |
| Glic bridge disconnect | "Connecting to AI Panel runtime... Retry" | Click Tabbit AI icon in sidebar |
| Panel ID changed | CDP returns 404/closed | Re-run `curl localhost:9222/json` |
| False-positive extraction | Response contains prompt text | Re-read baseline AFTER send, not before |

### Suitability Decision

| Use Case | Feasible? | Why |
|----------|-----------|-----|
| 10–20 quality samples | ✅ **Yes** | ~30–60 min, real code quality |
| 100+ samples | ⚠️ **Marginal** | ~5 hours, risk of bridge timeouts |
| 500+ samples | ❌ **No** | ~28 hours via CDP, use API models instead |

See `examples/trajectory_batch.py` for the full production script.
